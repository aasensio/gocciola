...
!-----------------------------------------------------------------
	subroutine formal_solver_gs_pp_overlap(which)
	integer :: which
...
! Make a formal solution of RTE using GS or SOR (population changes are made inside)		
		call rtpp_gs_overlap(n_radios,which)	

	end subroutine formal_solver_gs_pp_overlap
...
!-----------------------------------------------------------------------	
	...
	subroutine rtpp_gs_overlap(np, which)
	...
	real(kind=8) :: short_rev, mu_pp(3), wtmu(3)
...
! Only do it if the complete GS is being used		
	...
	end subroutine rtpp_gs_overlap
end module formal_gs_pp_overlap
