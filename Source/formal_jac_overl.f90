...
!-----------------------------------------------------------------
	subroutine formal_solver_jacobi_overlap
...
! Do the formal solution of the RTE
			call rtspher_jacobi_overlap(npmip1, it)	
...
! of the rate matrix
	...
	end subroutine formal_solver_jacobi_overlap
...
!-----------------------------------------------------------------------	
	subroutine rtspher_jacobi_overlap(np, itr)
...
!			Jbar20(itr,k) = Jbar20(itr,k) + FOURSQRTTWO * &
!				sum( (3.d0*mu(k,cortes+1)**2-1.d0) * wtmu * wtnu * perfil * In(:,cortes+1) )			
		endif
		
	enddo !k
	
	
	end subroutine rtspher_jacobi_overlap
...
!-----------------------------------------------------------------------	
	subroutine rtspher_jacobi_overlap_only_int(np, itr)
	integer, INTENT(IN) :: np, itr
	integer :: idir, dir, k, k0, k1, kdel, km, kp, frq, cortes, inout
	real(kind=8), dimension(nfrq) :: chim, chi0, chip, sm, s0, sp, dtm, dtp, exu
...
! Linear SC
	...
	end subroutine rtspher_jacobi_overlap_only_int
end module formal_jacobi_overlap
